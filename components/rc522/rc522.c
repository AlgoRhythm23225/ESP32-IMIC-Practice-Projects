...
uint8_t ReadRawRC(uint8_t Address) {
    ...
    uint8_t tx_data[2] = { ((Address << 1) & 0x7E) | 0x80, 0x00 };
    uint8_t rx_data[2];
    ...
    // Sử dụng rfid_spi toàn cục ở đây
    spi_device_transmit(rfid_spi, &t);
...
{
    spi_transaction_t t;
    memset(&t, 0, sizeof(t));       // Xóa sạch cấu trúc t để tránh giá trị rác

    // Chuẩn bị 2 byte dữ liệu: [Địa chỉ] [Giá trị]
    // RC522 Write: bit MSB (bit 7) phải bằng 0
    uint8_t tx_data[2];
    tx_data[0] = (Address << 1) & 0x7E; 
    tx_data[1] = value;

    t.length = 16;                  // Truyền 2 byte = 16 bit
    t.tx_buffer = tx_data;          // Dữ liệu gửi đi
    t.rx_buffer = NULL;             // Ghi thì không cần nhận dữ liệu về

    // Thực hiện truyền lệnh qua SPI
    esp_err_t ret = spi_device_transmit(rfid_spi, &t);
    
    if (ret != ESP_OK) {
        ESP_LOGE("RC522", "WriteRawRC failed!");
    }
}
